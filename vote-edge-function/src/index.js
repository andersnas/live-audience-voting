...
const SSE_SERVER_URL = "https://services.code4media.com/voterapp/api/vote";
const ORIGIN_URL = 'https://webservices.code4media.com/voterapp';
...
function getSSEServerURL() {
  try {
    ...
    return "https://services.code4media.com/voterapp/api/vote";
  }
}
    ...
    async function vote(option){
      ...
      try{
        const res=await fetch(BASE+'/api/vote',{
          ...
          body:JSON.stringify({option,token:getToken()})
        });
        if(res.ok){
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .actions{display:flex;gap:1rem;max-width:500px;flex-wrap:wrap}
...
<body>
  ...
  <script>
    ...
    async function fetchTotals(){
      try{
        const res=await fetch(ORIGIN+'/api/totals');
        const data=await res.json();
        updateUI(data.totals,data.total);
        setStatus('Updated','ok');
      }catch{setStatus('Failed to fetch totals','err');}
    }
    async function clearVotes(){
      if(!confirm('Clear all votes and voter tokens?'))return;
      try{
        const res=await fetch(ORIGIN+'/api/clear',{method:'POST'});
        const data=await res.json();
        updateUI({A:0,B:0,C:0,D:0},0);
        setStatus('Cleared '+data.cleared+' voter tokens','ok');
      }catch{setStatus('Failed to clear','err');}
    }
